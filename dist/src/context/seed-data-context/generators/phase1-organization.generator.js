...
let Phase1OrganizationGenerator = Phase1OrganizationGenerator_1 = class Phase1OrganizationGenerator {
    ...
    async generate(config) {
        ...
        console.log(...oo_oo(`1393275879_151_4_153_5_4`, `🔍 [Phase1] currentUserId 확인: ${config.currentUserId || 'undefined'}`));
        if (config.currentUserId) {
            this.logger.log(`✅ 현재 사용자를 모든 직원의 관리자로 설정 시작 (currentUserId: ${config.currentUserId})`);
            console.log(...oo_oo(`1393275879_158_6_160_7_4`, `✅ [Phase1] 현재 사용자를 모든 직원의 관리자로 설정 시작 (currentUserId: ${config.currentUserId})`));
            await this.현재_사용자를_모든_직원의_관리자로_설정한다(employeeIds, config.currentUserId);
            this.logger.log(`✅ 현재 사용자를 모든 직원의 관리자로 설정 완료`);
            console.log(...oo_oo(`1393275879_166_6_168_7_4`, `✅ [Phase1] 현재 사용자를 모든 직원의 관리자로 설정 완료`));
        }
        else {
            this.logger.log('⚠️ currentUserId가 없어 관리자 설정을 건너뜁니다.');
            console.log(...oo_oo(`1393275879_171_6_173_7_4`, '⚠️ [Phase1] currentUserId가 없어 관리자 설정을 건너뜁니다.'));
        }
    ...
    async 현재_사용자를_모든_직원의_관리자로_설정한다(employeeIds, currentUserId) {
        ...
        console.log(...oo_oo(`1393275879_951_4_953_5_4`, `[Phase1] 현재 사용자를 모든 직원의 관리자로 설정: ${employeeIds.length}명, currentUserId: ${currentUserId}`));
        const targetEmployeeIds = employeeIds.filter((id) => id !== currentUserId);
        console.log(...oo_oo(`1393275879_958_4_960_5_4`, `[Phase1] 대상 직원 수: ${targetEmployeeIds.length}명 (전체: ${employeeIds.length}명, 현재 사용자 제외)`));
        if (targetEmployeeIds.length > 0) {
            ...
            console.log(...oo_oo(`1393275879_973_6_975_7_4`, `✅ [Phase1] ${updateResult.affected}명의 직원에게 현재 사용자를 관리자로 설정 완료 (영향받은 행: ${updateResult.affected})`));
        }
        else {
            this.logger.log('⚠️ 설정할 직원이 없습니다 (모든 직원이 현재 사용자)');
            console.log(...oo_oo(`1393275879_978_6_980_7_4`, '⚠️ [Phase1] 설정할 직원이 없습니다 (모든 직원이 현재 사용자)'));
        }
    }
};
