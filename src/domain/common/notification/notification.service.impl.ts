 ...
 */
@Injectable()
export class NotificationServiceImpl implements INotificationService, OnModuleInit {
  ...
  async 초기화한다(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `알림 서비스 초기화 완료 (소요 시간: ${elapsedTime}ms)`,
      );
    } catch (error) {
      this.logger.error(
  ...
  async 알림목록을조회한다(params: GetNotificationsParams): Promise<GetNotificationsResult> {
    ...
    try {
      ...
      this.logger.log(
        `알림 목록 조회 성공: 알림 수=${response.data.notifications?.length || 0}`,
      );

      // 날짜 문자열을 Date 객체로 변환
      const notifications: NotificationInfo[] = (response.data.notifications || []).map((notification: any) => ({
        ...notification,
  ...
  async 알림을읽음처리한다(params: MarkNotificationAsReadParams): Promise<MarkNotificationAsReadResult> {
    ...
    try {
      ...
      const response = await this.httpClient.patch<MarkNotificationAsReadResult>(
