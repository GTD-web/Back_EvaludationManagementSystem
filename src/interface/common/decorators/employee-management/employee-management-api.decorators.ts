...
} from '../../dto/employee-management/department-hierarchy-response.dto';
import {
  PartLeadersResponseDto,
  EmployeeResponseDto,
} from '../../dto/employee-management/employee-management.dto';
...
/**
 * 직원 관리자 권한 변경 엔드포인트 데코레이터
 */
export function UpdateEmployeeAdmin() {
  return applyDecorators(
    Patch(':id/admin'),
    HttpCode(HttpStatus.OK),
    ApiOperation({
...
- 응답 데이터 검증: 변경된 isAdmin 값이 응답에 포함됨`,
    ...
    ApiResponse({
      status: 400,
      description:
        '잘못된 요청 데이터 (isAdmin 쿼리 파라미터 누락, 잘못된 값 또는 잘못된 UUID 형식)',
    }),
    ApiResponse({
      status: 404,
      description: '직원을 찾을 수 없습니다.',
...
/**
 * 여러 직원 관리자 권한 일괄 변경 엔드포인트 데코레이터
 */
export function BulkUpdateEmployeeAdmin() {
  return applyDecorators(
    Patch('bulk/admin'),
    HttpCode(HttpStatus.OK),
    ApiOperation({
...
- 응답 구조 검증: success, totalProcessed, succeeded, failed, failedIds, errors, processedAt 필드 모두 포함`,
    ...
    ApiResponse({
      status: 400,
      description:
        '잘못된 요청 데이터 (employeeIds 누락/빈 배열, 잘못된 UUID, isAdmin 파라미터 누락 등)',
    }),
    ApiResponse({ status: 500, description: '서버 내부 오류' }),
  );
}
