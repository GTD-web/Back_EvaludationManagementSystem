...
/**
 * 이전 평가기간 데이터 복사 엔드포인트 데코레이터 (Admin/Evaluator용)
 */
export function CopyPreviousPeriodData() {
  return applyDecorators(
    Post(':targetPeriodId/employees/:employeeId/copy-from/:sourcePeriodId'),
    HttpCode(HttpStatus.OK),
    ApiOperation({
...
- 각 WBS의 세부 프로젝트(subProject) 정보를 함께 반환합니다 (자기평가에 입력된 경우).
...
- projects: 프로젝트 및 WBS 할당 목록
  ...
  * wbsList: WBS 목록
    - subProject: 세부 프로젝트 (자기평가에 입력된 경우)
...
- 세부 프로젝트 포함: 자기평가에 입력된 subProject가 포함되어 반환됨
