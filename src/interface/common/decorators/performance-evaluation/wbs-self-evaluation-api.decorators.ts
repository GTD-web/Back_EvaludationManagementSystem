...
} from '../../dto/performance-evaluation/wbs-self-evaluation.dto';
...
- **Activity Log 연동**: 제출 후 평가자가 데이터 조회 시 viewedBy 필드 활성화 트리거
- 잘못된 UUID: UUID 형식이 아닌 ID로 요청 시 400 에러
- 존재하지 않는 ID: 유효한 UUID이지만 존재하지 않는 자기평가 ID로 요청 시 400 에러
...
export function ResetAllWbsSelfEvaluationsToEvaluatorByEmployeePeriod() {
  return applyDecorators(
    ...
    ApiOperation({
      summary:
        '직원의 전체 WBS 자기평가 한 번에 취소 (피평가자 → 1차 평가자 제출 취소)',
...
export function ResetWbsSelfEvaluationsToEvaluatorByProject() {
  return applyDecorators(
    Patch(
      'employee/:employeeId/period/:periodId/project/:projectId/reset-to-evaluator',
    ),
    HttpCode(HttpStatus.OK),
    ApiOperation({
...
export function SubmitWbsSelfEvaluationsToEvaluatorByProject() {
  return applyDecorators(
    Patch(
      'employee/:employeeId/period/:periodId/project/:projectId/submit-to-evaluator',
    ),
    HttpCode(HttpStatus.OK),
    ApiOperation({
