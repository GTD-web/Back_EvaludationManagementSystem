 ...
 */
@ApiTags('A-0-4. 평가자 - 평가 제출 관리')
@ApiBearerAuth('Bearer')
@Roles('evaluator')
@Controller('evaluator/evaluation-submission')
export class EvaluatorEvaluationSubmissionController {
  private readonly logger = new Logger(EvaluatorEvaluationSubmissionController.name);
   ...
   * - 평가기준: POST /evaluator/evaluation-submission/criteria
   * - 자기평가: POST /evaluator/evaluation-submission/self-evaluation/:employeeId/:periodId
   * - 1차 하향평가: POST /evaluator/evaluation-submission/primary-downward/:evaluateeId/:periodId/:wbsId
   * - 2차 하향평가: POST /evaluator/evaluation-submission/secondary-downward/:evaluateeId/:periodId/:wbsId
   */
  @UpdateEvaluationSubmission()
